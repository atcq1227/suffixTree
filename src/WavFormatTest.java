/* Appropiated from Tobias Amdahl by Matt Shaw
 * Found at https://thiscouldbebetter.wordpress.com/2011/08/14/reading-and-writing-a-wav-file-in-java/
 */
...
public class WavFormatTest
{
    ...
    {
    	readAndRewriteWav("footsteps");
    }    

    //format the input so that it can be read
...
//provides an input stream in little endian format
...
//provides an output strema in little endian format
...
//all of the characteristics and methods for working with the wav file
class WavFile
{
    ...
    //read from file path specified in main()
    ...
    //read little endian chunks from file path specified in main()
    ...
    //provides info from reading chunks
    ...
    //provide the data read from chunks
    ...
    //write data to path after manipulation
    ...
    //write chunks to file path after manipulation
    ...
    //provides info and format for writing chunks
    ...
    //provides data for reading and writing chunks
    ...
    //abstract class for individual samples in each file
    public static abstract class Sample
    {
        ...
        public static <E> Sample[][] buildManyFromBytes
        (
                SamplingInfo samplingInfo,
                byte[] bytesToConvert
        )
        {
        	...
        	//output length (in bytes) for the file to be written. more than 3000 will take quite awhile
        	int outputLength = 1000000;
        	
        	//find all the necessary info from sample info above
            ...
            //create array to return bytes from sample
            ...
            //using Byte wrapper to gives bytes to SuffixTree class
            ArrayList<Byte> byteArrayForPST = new ArrayList<Byte>();
            
            //initialize a suffix tree with 4 rows
            SuffixTree<Byte> bytePST = new SuffixTree<Byte>(4);
            
            //read through each sample on each channel and put each byte into arrayList
            ...
            {                
                for (int c = 0; c < numberOfChannels; c++)
                {
                    for (int i = 0; i < bytesPerSample; i++)
                    {
                    	...
                    	if(b < outputLength) {
                    		byteArrayForPST.add(bytesToConvert[b]);
                    	}
                        b++;
                    }
            ...
            {                
                for (int c = 0; c < numberOfChannels; c++)
                {
                    for (int i = 0; i < bytesPerSample; i++)
                    {
                    	if(b < bytePST.getSingleGeneratedElements().size()) {
	                    	sampleValueAsBytes[i] = bytePST.getSingleGeneratedElements().get(b);
	                        b++;
                    	}
                    }
        ...
        //return bytes from sample class as array
